...
use async_openai::{
	config,
	types::{
		AssistantObject, CreateAssistantFileRequest, CreateAssistantRequest, CreateFileRequest,
		CreateRunRequest, CreateThreadRequest, ModifyAssistantRequest, RunStatus, ThreadObject,
	},
};
use console::Term;
...
pub async fn load_or_crate_asst(
	oac: &OaClient,
	config: CreateConfig,
	recreate: bool,
) -> Result<AsstId> {
...
pub async fn first_by_name(oac: &OaClient, name: &str) -> Result<Option<AssistantObject>> {
	let oa_assists = oac.assistants();

	let assts = oa_assists.list(DEFAULT_QUERY).await?.data;
...
//region ----threads
pub async fn create_threads(oac: &OaClient) -> Result<ThreadId> {
