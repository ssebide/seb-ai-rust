...
///Returns true if one or more dirs was created
...
pub fn list_files(
	...
	let exclude_globs = include_globs
		.map(|arg0: GlobSet| get_glob_set(/* &[&str] */))
		.transpose()?;
	...
	let paths = walk_dir_it.map(|e| e.path());
