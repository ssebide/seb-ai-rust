...
use crate::ais::{OaClient, new_oa_client};
...
impl Buddy {
	...
	pub async fn init_from_dir(dir: impl AsRef<Path>, recreate_asst: bool) -> Result<Self> {
		...
		let asst_id = asst::load_or_crate_asst(&oac, (&config).into(), recreate_asst).await?;
	...
	pub async fn upload_instructions(&self) -> Result<bool> {
		...
		if file.exists() {
			...
			asst::upload_instruction(&self.oac, &self.asst_id, inst_content).await?;
	...
	pub async fn load_or_create_conv(&self, recreate: bool) -> Result<Conv> {
		...
		let conv = if let Ok(conv) = load_from_json::<Conv>(&conv_file) {
			...
			let thread_id = asst::create_threads(&self.oac).await?;
